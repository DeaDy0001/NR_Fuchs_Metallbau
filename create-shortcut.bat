...
if not exist "frontend\nr_logo\NR_Logo_150x150.ico" (
    echo [1/2] Erstelle Icon aus NR_Logo_150x150.png...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "Add-Type -AssemblyName System.Drawing; $img = [System.Drawing.Image]::FromFile('%~dp0frontend\nr_logo\NR_Logo_150x150.png'); $bmp = New-Object System.Drawing.Bitmap($img, 256, 256); $icon = [System.Drawing.Icon]::FromHandle($bmp.GetHicon()); $stream = [System.IO.File]::Create('%~dp0frontend\nr_logo\NR_Logo_150x150.ico'); $icon.Save($stream); $stream.Close(); $icon.Dispose(); $bmp.Dispose(); $img.Dispose(); Write-Host '[OK] Icon erstellt: frontend\nr_logo\NR_Logo_150x150.ico'"
    if %ERRORLEVEL% NEQ 0 (
